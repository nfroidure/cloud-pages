...
describe('cloudPages', () => {
  ...
  it('should fail with no files found', () =>
    $.run(['ENV', 'cloudPages', 'glob'])
    .then(({ ENV, cloudPages, glob }) => {
      ENV.AWS_S3_BUCKET = 'lol';
  ...
  it('should work with a file found', () =>
    $.run(['ENV', 'cloudPages', 'glob', 'git', 's3', 'fs', 'mime'])
    .then(({ ENV, cloudPages, glob, git, s3, fs, mime }) => {
      ...
      git.getCommits.returns(Promise.resolve([{
        tag: 'v3.0.1',
